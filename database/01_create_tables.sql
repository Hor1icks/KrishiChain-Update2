...
CREATE INDEX IX_BATCH_ARAT           ON HARVEST_BATCH (AratID);
CREATE INDEX IX_BATCH_STATUS         ON HARVEST_BATCH (Status, AratID);

CREATE INDEX IX_WAREHOUSE_MANAGER    ON WAREHOUSE (ManagerID);
CREATE INDEX IX_STORES_BATCH         ON STORES (BatchID);
CREATE INDEX IX_STORES_MANAGER       ON STORES (ManagerID);
CREATE INDEX IX_STORES_OPEN          ON STORES (WarehouseID, UnitNo, DateOut);
...
CREATE INDEX IX_ORDER_DATE           ON SALE_ORDER (OrderDate);

CREATE INDEX IX_PAYMENT_ORDER        ON PAYMENT (SaleOrderID);
...
CREATE INDEX IX_ASSIGNED_PERSONNEL   ON ASSIGNED_TO (PersonnelID);

CREATE INDEX IX_PRICE_DATE           ON DAILY_MARKET_PRICE (PriceDate, CropID);
CREATE INDEX IX_DMP_ARAT             ON DAILY_MARKET_PRICE (AratID);
