...
/**
 * The manager's side of the storage-consent workflow.
 *
 * A manager never puts produce into a unit unilaterally — they *propose*
 * an allocation (PRD §9.10 transaction #2) and the customer accepts or
 * rejects it. So this page is a proposal desk, not an allocation form:
 * two queues of things that could be stored, and a ledger of proposals
 * at every stage of their life.
 *
 * The two queues are the two storage legs, and they are genuinely
 * different entities — leg 1 stores a farmer's unsold *batch* before
 * auction, leg 2 stores a buyer's purchased *sale order* after it. They
 * are kept apart here because the customer, the identifier and the
 * quantity ceiling all differ.
 */
...
export default function Allocations() {
  ...
  // An open counter-offer being composed against one incoming request.
  const [countering, setCountering] = useState(null);
  const [counterRate, setCounterRate] = useState('');

  // What is being proposed, and on what terms. `placing.leg` is 1 or 2.
  ...
  // The warehouse's per-kg rate is snapshotted onto the allocation at
  // proposal time, so the fee the customer will see is knowable here.
  ...
  // No point pricing a quantity we are about to refuse.
  const estimatedFee =
  ...
  return (
    <div className="page">
      ...
      {/* Everything the manager owes an answer to. Two shapes arrive here
          — a customer asking for space, and a customer countering the
          manager's own rate — and `awaiting` tells them apart. */}
      {requests.length > 0 && (
        <>
          ...
          <table>
            ...
            <tbody>
              {requests.map((r) => {
                ...
                // A counter is settled at their rate; a fresh request is
                // still at the warehouse's own rate.
                  ...
                  <tr key={r.allocationId}>
                    ...
                    <td>
                      <div className="actions" style={{ margin: 0 }}>
                        ...
                        {/* Only a fresh request can be countered. A counter
                            is the last word — countering it back would make
                            the negotiation endless, so the server refuses. */}
                        {!isCounter && (
                          <button
                            type="button"
                            className="small ghost"
      ...
      {visible.length === 0 ? (
        ...
        <table>
          ...
          <tbody>
            {visible.map((a) => {
              // A fee is only owed once the customer has actually accepted
              // the terms — the same rule the server enforces in payFee().
              // Showing "owing" against a PENDING_ACCEPT or COUNTERED row
              // would claim money nobody has agreed to yet.
              ...
              const owed = feeIsDue
                ...
                : 0;
              // PENDING_RELEASE is only ours to answer when the *other*
              // party raised it; otherwise we are the ones being waited on.
              const oursToAnswer =
                a.allocationStatus === 'PENDING_RELEASE' && a.releaseRequestedBy !== 'MANAGER';
              // Whose turn it is depends on who spoke last, which differs
              // per status: ProposedBy for an unanswered offer, CounteredBy
              // once a counter is on the table, ReleaseRequestedBy for a
              // release. Anything in OUR court is actioned in the queue
              // above, so here it just points there.
